...
def plot_error_and_fragment_bias(samfile_path, alignments = 6000000):
    ...
    plt.grid(True)
    plt.savefig('/home/jyeh/summer2024/no_pcr_fragmentation_bias_alignment_start_position_distribution.png')  # Save the figure as PNG
    ...
    plt.grid(True)
    plt.savefig('/home/jyeh/summer2024/no_pcr_fragmentation_bias_alignment_end_position_distribution.png')  # Save the figure as PNG
    plt.close()  # Close the current figure to release memory
...
def plot_qual_scores_distribution(samfile_path, alignments=6000000):
    ...
    plt.grid(True)
    plt.savefig('/home/jyeh/summer2024/Quality_scores_distribution.png')
    plt.close()
...
def plot_fragment_coverage(samfile_path, alignments=6000000):
    ...
    plt.grid(True)
    plt.savefig('/home/jyeh/summer2024/no_pcr_fragment_coverage.png')
    plt.close()
    
    # coverage = dict(sorted(coverage.items(), key=lambda item: item[1]))
    
    # #write coverage to a file
    # with open('/u/c/yehjose1/summer2024/optimized_pcr_fragment_coverage_6cycles.txt', 'w') as f:
    ...
    plt.grid(True)
    plt.xlim(0, 1)
    plt.savefig('/home/jyeh/summer2024/no_pcr_fragment_coverage_vs_gc_content.png')
    plt.close()
            
    # get entropy of coverage
...
def plot_error_distribution(samfile_path, alignments=6000000):
    ...
    plt.grid(True)
    plt.savefig('/home/jyeh/summer2024/no_pcr_error_distribution.png')
    plt.close()
...
def open_dict_txt(fwd_dict_file, rvs_dict_file):
    fwd_dict = {}
    with open(fwd_dict_file, 'r') as f:
        for line in f:
            if len(line.strip()) == 9:
                key = line.strip()
                fwd_dict[key] = []
            else:
                fwd_dict[key].append(line.strip())
                
    rvs_dict = {}
    with open(rvs_dict_file, 'r') as f:
        for line in f:
            if len(line.strip()) == 9:
                key = line.strip()
                rvs_dict[key] = []
            else:
                rvs_dict[key].append(line.strip())
    
    return fwd_dict, rvs_dict

def unique_seq_counter(fwd_dict, rvs_dict):
    fwd_seqs = set()
    for key in fwd_dict:
        unique = True
        for seq in fwd_dict[key]:
            if seq != fwd_dict[key][0]:
                unique = False
        if unique == True:
            fwd_seqs.add(fwd_dict[key][0])   
    
    rvs_seqs = set()
    for key in rvs_dict:
        unique = True
        for seq in rvs_dict[key]:
            if seq != rvs_dict[key][0]:
                unique = False
        if unique == True:
            rvs_seqs.add(rvs_dict[key][0])
    
    print(f"Keys with Unique fwd sequences: {len(fwd_seqs)}")
    print(f"Keys with Unique rvs sequences: {len(rvs_seqs)}")
    return fwd_seqs, rvs_seqs
...
if __name__ == '__main__':
    ...
    # samfile_path = "/u/c/yehjose1/summer2024/test_SRA_output.sam"
    ...
    # samfile_path = "/u/c/yehjose1/summer2024/optimized_pcr_aligned_16cycles.sam"
